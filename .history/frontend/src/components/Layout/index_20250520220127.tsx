...
import { getAllPatternsRoute, getSignInRoute, getSignOutRoute, getSignUpRoute, getTypePatternRoute } from '../../lib/routes';
    ...
    <div className={css.layout}>
      <div className={css.navigation}>
        ...
        {isLoading || isFetching || isError ? null : data.authorizedUser ? (
          <>
            <p className={css.user}>{data.authorizedUser.nick}</p>
            <Link className={cn(css.link, css.btn)} to={getSignOutRoute()}>
              Выход
            </Link>
          </>
        ) : (
          <>
            <Link className={cn(css.link, css.btn)} to={getSignInRoute()}>
              Вход
            </Link>
            <Link className={cn(css.link, css.btn)} to={getSignUpRoute()}>
              Регистрация
            </Link>
          </>
        )}
